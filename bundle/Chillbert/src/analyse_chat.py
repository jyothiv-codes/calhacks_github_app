...
class AnalyseChat:
    ...
    def __init__(self, user):
        ...
        self.user = user
        self.cb = ChatBot(self.user)

    def store_ac(self):

        print('Setting up Chat Analysis')
        output_file = self.fp.processFile()
        summary = self.cb.summarize(output_file)
    ...
    def sendChatSummary(self, summary_message):
        msg = f'Your patient {self.user} recently spoke to me. Here is the summary of our conversation. \n{summary_message}'
        self.m.sendMessage(msg, os.environ['TWILIO_RECIPIENT'])

    def sendHelpfulLinks(self, link_message):
        msg = f'I really enjoyed our converation today {self.user}. Based on our chat, here are a few helpful links for you!\n{link_message}'
        self.m.sendMessage(msg, os.environ['TWILIO_RECIPIENT'])
